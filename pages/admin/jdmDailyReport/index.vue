...
async function saveMemo() {
  ...
  const memoList = items.map(item => ({...memoListMapper(item), certDate: criteria.certDate, type: 0, agency: criteria.agency }))
...
async function PrintDailyReport() {
  try {
    ...
    const { status } = downloadFile(`${baseURL}/api/v1/jdmDailyReport/PrintDailyReport?agency=${criteria.agency.trim()}
