      ...
      <el-col :span="24">
        ...
        <el-card header="查詢結果" class="mt-4">
          ...
          <el-table :data="queryResult.items" stripe style="width: 100%">
            ...
            <el-table-column prop="signReportker" label="報告繕打者" align="center">
              <template #default="{ row }">
                <ClientOnly v-for="(signReportker) in row.signReportker" :key="signReportker">
                  <el-col :span="24">
                    <a v-if="signReportker === '0'">看不到此筆</a>
                    <el-button
                      v-if="signReportker === '1'" type="primary"
                      @click="navigateTo({ path: '/admin/entrustOrderApplication/Edit' })"
                    >
                      <AdminIcon name="i-carbon:edit" class="pr-1" />
                      編輯
                    </el-button>
                    <el-button v-if="signReportker === '2'" type="danger">
                      <AdminIcon name="i-carbon:delete" class="pr-1" />
                      刪除
                    </el-button>
                    <el-button v-if="signReportker === '3'" type="warning">
                      <AdminIcon name="i-carbon:arrow-right" class="pr-1" />
                      開始呈核
                    </el-button>
                    <el-button v-if="signReportker === '4'" type="warning">
                      <AdminIcon name="i-carbon:airline-rapid-board" class="pr-1" />
                      呈核
                    </el-button>
                    <el-button
                      v-if="signReportker === '5'" type="success"
                      @click="navigateTo({ path: '/admin/entrustOrderApplication/Edit' })"
                    >
                      <AdminIcon name="i-carbon:view" class="pr-1" />
                      瀏覽
                    </el-button>
                    <el-button v-if="signReportker === '6'" type="primary">
                      <AdminIcon name="i-carbon:connection-receive" class="pr-1" />
                      領取報告
                    </el-button>
                    <el-button v-if="signReportker === '7'" type="warning">
